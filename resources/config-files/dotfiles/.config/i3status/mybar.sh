...
mycrypto() {
  ...
  echo -n "\"full_text\":\"  \","
	#echo -n "\"full_text\":\"     $(/home/alba/.config/i3status/crypto.py)     \","
  echo -n "\"color\":\"#000000\","
...
myvpn_on() {
  local bg="#b5bd68" # grey darken-3
  local icon=""
  if [ -d /proc/sys/net/ipv4/conf/proton0 ]; then
    bg="#E53935" # rouge
    icon=""
  fi
  separator $bg "#454a4f" # background left previous block
